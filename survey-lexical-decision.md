...
description: "This document catalogs papers and datasets on lexical decision reaction times (LD-RTs). A central finding across this literature is that word frequency (or unigram surprisal) is generally the single most powerful predictor, typically accounting for 30--40% of variance, while modern distributional and neural models contribute to explaining additional variance through semantic neighborhood, morphological transparency, and tokenization-derived features."
date: 2026-03-02
...
::: {.callout-important title="Gym interfaces for LD-RT evaluation"}

The Psycholinguistics Gym for lexical decision needs to support two kinds of model submission and a standardized data layer.
...
4. **RT distribution quantiles.**
    ...
    - **Gym interface:** The submitted model (typically a @sec-type12 process model) generates predicted quantiles; evaluation via $\chi^2$ fit across quantile bins, or quantile regression.

5. **Ex-Gaussian parameters.**
...
## Lexical decision accuracy {#sec-accuracy}
...
## Primed lexical decision {#sec-primeddv}
...
## Auditory lexical decision {#sec-auditorydv}
...
## Naming / pronunciation {#sec-namingdv}
...
1. **ERP amplitudes (N400, P600).**
    ...
    - **Papers:** @sassenhagen2020traces (encoding/decoding of fastText vectors during isolated word reading).
...
# Taxonomy of model types by predictor type {#sec-taxonomy}
...
Deterministic properties that can be computed from the word string alone, or from the word string plus a reference lexicon. A researcher can compute these in a few lines of code given only the stimulus list and standard resources. They require no "model" in the machine learning sense---no training, no optimization, no learned weights.

### Intrinsic word properties {#sec-type0}
...
### Orthographic and phonological neighborhood statistics {#sec-type1}
...
## Corpus-derived frequency and diversity statistics {#sec-corpus-freq}

Word frequency is the single most powerful predictor of LD RT, typically accounting for 30--40% of variance, and every paper in this survey controls for it. Unlike the purely deterministic features above, frequency statistics require a text corpus and the choice of corpus matters enormously---subtitle-based frequencies consistently outperform written-text frequencies across languages. Despite being simple corpus counts, these features serve as the mandatory baseline that any more sophisticated model must improve upon.

### Corpus-derived frequency and diversity {#sec-type2}
...
### Surprisal and language model probabilities {#sec-type11}
...
## Human-normed and LLM-generated psycholinguistic variables {#sec-human-normed}
...
A recent development is the use of large language models (LLMs) to *simulate* human norming at scale (see @sec-type5). LLMs prompted to rate word familiarity or concreteness produce norms that correlate highly with human norms and, in some cases, predict LD RT better than the human norms themselves. At the extreme end, fine-tuned LLMs can directly output predicted RTs, collapsing the entire predictor → linking function → RT pipeline into a single black-box model.

For the Gym, these predictors are distinguished from [word-intrinsic and lexicon-derived features](#sec-word-intrinsic) by the fact that their values cannot be recomputed from the stimulus list alone---they must be *supplied* by the user (or by the Gym's norm database) as fixed tables.

### Human-collected psycholinguistic norms {#sec-type3}
...
### LLM-generated norms and simulated RTs {#sec-type5}
...
## Trained representation models {#sec-trained-rep}

These are models in the machine learning sense: they have internal parameters (weights, embeddings, vocabulary tables) that have been *learned* from a corpus of text or from form-meaning mappings. Given a word string as input, they produce a structured representation---a dense vector, a probability distribution, an activation pattern, or a set of derived scalar features---that encodes what the model has learned about that word from its training data. Unlike [word-intrinsic features](#sec-word-intrinsic), [corpus-derived frequencies](#sec-corpus-freq), or [human-normed lookup tables](#sec-human-normed), these models transform the input through learned parameters, and different training corpora, architectures, or hyperparameters will produce different representations for the same word.

The key distinction from [process and decision models](#sec-process-decision) (below) is that trained representation models produce *representations or features*, not *behavioral predictions*. They output something like "the embedding vector for *cat* is [0.23, -0.41, ...]" or "the tokenizer segments *unhappiness* into [un, happi, ness]"---it is then the Gym's evaluation pipeline (or the user's separate linking function) that converts these representations into predicted RTs. In other words, these models answer "what does the model represent about this word?" while process models answer "how does the system make a word/nonword decision?"

This group spans a wide range of complexity, from simple embedding lookups to discrimination-based learning models. They are ordered roughly by the richness of the input they require and the complexity of their internal computation.

### Static word embedding models {#sec-type6}
...
### Word association / network models {#sec-type7}
...
- **Gym interface:** `word_string → {assoc_strength, centrality, ...}`. Structurally similar to @sec-type6 but the underlying representation is a graph rather than a vector space.

### Tokenization-derived features {#sec-type8}
...
### Compositional distributional models (morphology) {#sec-type9}
...
- **Gym interface:** `(stem_vector, affix) → composed_vector; cosine(composed, observed) → transparency_score`. Requires the user to provide both the composition function and the base embedding space. Inherits from @sec-type6.

### Discrimination-based models {#sec-type10}
...
Repositories of isolated-word human behavioral data (RTs, accuracy) for visual or auditory lexical decision. These serve as the primary benchmarks against which all models are evaluated.


1. **English Lexicon Project (ELP).** @balota2007elp

    - 40,481 words + 40,481 nonwords; 816 participants (LD), 444 (naming); American English; visual LD (uppercase) + speeded naming. Available at elexicon.wustl.edu.
    - **DVs:** Mean RT, zRT, accuracy; also naming RT.
    - **Predictors reported:** Log HAL frequency, Kučera--Francis frequency, orthographic N, word length (letters, syllables, morphemes), phonological N, bigram frequency.
    - **Key findings:** Established the megastudy approach. Word frequency is the dominant predictor (~42--45% of LD RT variance for monosyllabic words). Enabled post-hoc testing of any new predictor against a massive behavioral dataset.

2. **British Lexicon Project (BLP).** @keuleers2012blp

    - 28,730 words + nonwords; 78 participants (repeated measures); British English; visual LD (lowercase); nonwords generated by Wuggy [@keuleers2010wuggy]. Available at crr.ugent.be/blp.
    - **DVs:** Mean RT, zRT, accuracy.
    - **Predictors reported:** SUBTLEX-US frequency, BNC frequencies, AoA, familiarity, imageability.
    - **Key findings:** Megastudy results are robust across American vs. British English and uppercase vs. lowercase presentation. Frequency accounts for ~38% of RT variance. Correlation with ELP: $r \approx .74$ for zRTs.

3. **Dutch Lexicon Project (DLP1).** @keuleers2010dlp

    - 14,000 words + 14,000 nonwords; 39 participants (fully repeated measures); Dutch; visual LD (lowercase).
    - **DVs:** Mean RT, accuracy.
    - **Predictors reported:** CELEX frequency, SUBTLEX-NL frequency, word length, orthographic N.
...
4. **Dutch Lexicon Project 2 (DLP2).** @brysbaert2016dlp2

    - 30,000 Dutch lemmas; ~81 participants; Dutch; visual LD.
    - **DVs:** Mean RT, zRT, accuracy.
    - **Predictors reported:** Word prevalence (% population knowing word), SUBTLEX-NL frequency, AoA, OLD20, concreteness.
...
5. **French Lexicon Project (FLP).** @ferrand2010flp

    - 38,840 French words + 38,840 pseudowords; 975 participants; French; visual LD (lowercase).
    - **DVs:** Mean RT, accuracy.
    - **Predictors reported:** Lexique database frequency, word length.
...
6. **MEGALEX.** @ferrand2018megalex

    - 28,466 French words (visual LD) + 17,876 words (auditory LD); ~1,000 participants; French.
    - **DVs:** Mean RT, accuracy (both visual and auditory).
    - **Predictors reported:** Five frequency measures (Lexique, SUBTLEX, Worldlex), OLD20, syllable count.
    - **Key findings:** First megastudy enabling direct visual vs. auditory comparison. Worldlex frequencies (Twitter + blogs + newspapers) explained slightly more variance (~48.5%) than subtitle frequencies (~47.6%) for visual LD.

7. **MELD-SCH.** @tsang2018meldsch

    - 12,578 words; 504 participants; Simplified Chinese (Mandarin); visual LD.
    - **DVs:** Mean RT, accuracy.
    - **Predictors reported:** SUBTLEX-CH frequency, contextual diversity, stroke count, radical frequency, semantic transparency.
...
8. **Chinese Lexicon Project (CLP).** @tse2017clp

    - 25,000+ Traditional Chinese two-character compound words; ~594 Cantonese speakers; visual LD.
    - **DVs:** Mean RT, accuracy.
    - **Predictors reported:** SUBTLEX-CH subtitle frequency, contextual diversity, stroke count, character frequency.
...
9. **SPALEX.** @aguasvivas2020spalex

    - ~45,000 Spanish words; 150,000+ participants (crowdsourced); 20 Spanish-speaking countries; visual LD.
    - **DVs:** Mean RT, accuracy.
...
10. **Italian Crowdsourcing Project (ICP).** @amenta2025italian

    - 130,465 Italian words; 156,000+ participants; Italian; online visual LD.
    - **DVs:** Mean RT, accuracy.
    - **Key findings:** ICP RTs correlate $r = .78$ with lab-based LD latencies. Word prevalence effects replicated for Italian.

11. **English Crowdsourcing Project (ECP).** @mandera2020ecp

    - 62,000 English words; >1 million participants; English; online visual LD.
    - **DVs:** Mean RT.
    - **Key findings:** Provides valid RTs for ~35,000 words not in ELP. Correlation with ELP: $r \approx .75$.

12. **Massive Auditory Lexical Decision (MALD).** @tucker2019mald

    - 26,793 words + 9,592 pseudowords; 231 participants; English; auditory LD.
    - **DVs:** Mean RT, accuracy.
    - **Predictors reported:** SUBTLEX frequency, phonological neighborhood density, uniqueness point, phonotactic probability, duration, phoneme/syllable count.
...
13. **Semantic Priming Project (SPP).** @hutchison2013spp

    - 6,641 prime--target pairs (300 targets); 768 participants; two SOAs (200 ms, 1200 ms); English; semantic priming with LD and naming.
...
## Word-intrinsic and lexicon-derived features {#sec-lit-word-intrinsic}

### Intrinsic word properties and orthographic neighborhood {#sec-lit-types01}

1. **Word Length Effects.** @new2006wordlength

    - **Dataset:** ELP (~33,006 words, 3--13 letters).
    - **Human DV:** LD RT.
    - **Predictors:** Letter count, syllable count, orthographic N, log HAL frequency.
...
2. **OLD20.** @yarkoni2008old20

    - **Dataset:** ELP (monosyllabic subset ~2,906 words + full ~40K).
    - **Human DV:** LD RT, naming RT.
    - **Predictors:** OLD20 = mean Levenshtein distance to 20 nearest orthographic neighbors (includes additions, deletions, transpositions across lengths). Compared with Coltheart's N.
...
3. **Orthographic Neighborhood Size (N).** @coltheart1977access

    - **Predictors:** Orthographic N = number of words formed by single-letter substitution, preserving position and length.
...
4. **Neighborhood Effects Review.** @andrews1997neighborhood
...
## Corpus-derived frequency, diversity, and surprisal {#sec-lit-corpus-freq}

### Corpus-derived frequency and diversity {#sec-lit-type2}
...
1. **SUBTLEX-US Frequency Norms.** @brysbaert2009subtlexus
    ...
    - **Corpus/source:** Individual words; subtitle corpus.
    - **Predictors:** Word frequency per million (SUBTLEXWF), contextual diversity (SUBTLEXCD = % of films containing word), log-transformed versions.
...
3. **Contextual Diversity.** @adelman2006cd

    - **Dataset:** Three naming + three LD datasets (including Balota et al., 2004 monosyllabic set).
    - **Human DV:** LD RT, naming RT.
    - **Corpus/source:** Individual words; TASA, BNC, HAL corpora.
    - **Predictors:** Contextual diversity (CD = proportion of documents containing word) vs. raw frequency.
...
4. **The Rank Hypothesis.** @murray2004rank

    - **Human DV:** LD RT, error rates.
    - **Predictors:** Word frequency rank (ordinal position in frequency-ordered list) vs. log frequency.
...
5. **Frequency and the Decision Stage.** @balota1984frequency

    - **Human DV:** RT across category verification, LD, and pronunciation tasks.
    - **Predictors:** Word frequency (Kučera--Francis), length.
...
6. **Lexical Access and Naming Time.** @forster1973lexical
...
7. **Social Media Frequency.** @herdagdelen2017social

    - **Dataset:** ELP + BLP.
    - **Human DV:** LD RT.
    - **Predictors:** Word frequencies from Facebook and Twitter corpora.
...
8. **Word Frequency in German.** @brysbaert2011german

    - **Dataset:** German LD data (dlexDB and others).
    - **Predictors:** CELEX, Leipzig, dlexDB, Google Books, SUBTLEX-DE frequencies.
...
9. **Semantic Distinctiveness.** @johns2022distinctiveness [see also @jones2012diversity]

    - **Dataset:** ELP, BLP, DLP.
    - **Human DV:** LD RT, accuracy, naming RT.
    - **Model output:** Semantic distinctiveness (SD), user-based contextual diversity (UCD-SD) from Reddit; counts unique vs. redundant contexts.
...
### Surprisal and language model probabilities {#sec-lit-type11}

1. **Cumulative Surprisal in Auditory LD.** @balling2012surprisal
...
## Human-normed and LLM-generated psycholinguistic variables {#sec-lit-human-normed}

### Human-collected psycholinguistic norms {#sec-lit-types34}

1. **Age of Acquisition Norms.** @kuperman2012aoa
...
2. **Emotion Effects.** @kuperman2014emotion
...
4. **Word Prevalence.** @keuleers2015prevalence
    ...
    - **Predictors:** Word prevalence = % of population knowing a word.
...
### LLM-generated norms and simulated RTs {#sec-lit-type5}

1. **GPT-FAM.** @brysbaert2025gptfam
...
3. **LLM-Augmented Psycholinguistic Datasets.** @trott2024augment
...
## Trained representation models {#sec-lit-trained-rep}

### Static word embeddings {#sec-lit-type6}
...
1. **Word2Vec vs. Count Models.** @mandera2017explaining
    ...
    - **Model input:** Individual words; word2vec trained on OpenSubtitles corpus.
    - **Model output:** Cosine similarity between word vectors; semantic neighborhood density (SND).
    - **Evaluation:** $R^2$ in multiple regression with covariates (frequency, length, etc.).
    - **Models tested:** Word2Vec CBOW, Word2Vec Skip-gram, GloVe, LSA, BEAGLE, HAL, PPMI-SVD.
...
2. **HiDEx.** @shaoul2010hidex

    - **Dataset:** ELP.
    - **Human DV:** LD RT, semantic decision RT.
    - **Model output:** Semantic neighborhood density (NCOUNT), average radius of co-occurrence (ARC), from HiDEx (extension of HAL model) trained on USENET.
...
3. **subs2vec.** @vanparidon2021subs2vec

    - **Dataset:** ELP, BLP, DLP, and megastudies in other languages.
    - **Human DV:** LD RT.
    - **Model input:** Individual words; fastText Skip-gram trained on OpenSubtitles.
    - **Model output:** SND (cosine similarity in embedding space), word frequency.
    - **Models tested:** fastText Skip-gram embeddings in 55 languages.
...
4. **Nonword Processing in BLP.** @hendrix2021nonword

    - **Dataset:** BLP (18,547 words + 27,079 nonwords).
    - **Human DV:** LD latencies (individual trial-level, analyzed as hazard functions).
    - **Model input:** Words and nonwords through fastText (subword n-grams allow nonword vectors).
    - **Model output:** Frequency (applicable to nonwords via Google frequency), SND, orthography-to-semantics consistency (OSC).
    - **Evaluation:** Piecewise additive mixed models (PAMMs)---survival analysis.
    - **Models tested:** fastText (uniquely allows nonword vector computation via subword architecture).
...
6. **Embeddings and Primed LD.** @auguste2017embeddings

    - **Dataset:** Semantic Priming Project [SPP; @hutchison2013spp].
    - **Human DV:** Primed LD RT.
    - **Model input:** Word pairs (prime, target).
    - **Model output:** Cosine similarity between embedding vectors.
    - **Evaluation:** Spearman correlation between cosine similarity and priming RT.
    - **Models tested:** GloVe (6B, 42B, 840B), Word2Vec (Google News, custom BNC, Wikipedia, OpenSubtitles).
...
7. **BEAGLE and Priming.** @jones2006beagle

    - **Human DV:** Priming effects (RT differences).
    - **Models tested:** BEAGLE, LSA, HAL.
...
8. **LSA Cosines Predict Priming.** @guenther2016lsa

    - **Human DV:** LD RT in priming experiment.
    - **Model output:** LSA cosine similarity (directly manipulated as IV).
...
### Word association and network models {#sec-lit-type7}

1. **SWOW-EN Word Associations.** @dedeyne2019swow
    ...
    - **Model output:** Associative strength, cosine similarity in PPMI space, spreading activation measures.
...
### Tokenization-derived features {#sec-lit-type8}

1. **Cognitive Plausibility of Subword Tokenization.** @beinborn2023tokenization

    - **Dataset:** ELP, BLP, DLP, FLP, SPALEX, and datasets for 15 languages total.
    - **Paradigm:** Visual LD.
    - **Human DV:** LD RT and accuracy (words and nonwords).
    - **Model input:** Isolated words/nonwords processed by tokenizer algorithms.
    - **Model output:** Number of subword tokens produced; "chunkability" metric (normalized token count).
    - **Evaluation:** Pearson/Spearman correlations between token count and RT/accuracy across four conditions.
    - **Tokenizers tested:** BPE (GPT-2), WordPiece (BERT), UnigramLM (XLNet/T5) at vocabulary sizes 1K--100K.
...
### Compositional distributional models {#sec-lit-type9}
...
1. **FRACSS Model.** @marelli2015fracss
    ...
    - **Model input:** Stem vectors + affix matrices from distributional semantic model.
    - **Model output:** Compositional derived-word vector; semantic transparency = cosine between composed and whole-word vectors.
...
2. **Compound Transparency.** @guenther2019compound

    - **Human DV:** LD RT.
    - **Model output:** CAOSS model (Compounding as Abstract Operation in Semantic Space)---compositionality = cosine between predicted and observed whole-word vector.
...
3. **Compound Word Megastudy.** @kim2019compound

    - **Dataset:** 2,861 English compounds; ELP LD and naming data.
    - **Human DV:** LD RT, naming RT.
    - **Model output:** Human-judged and distributional semantic transparency measures [@mandera2017explaining].
...
4. **German Compound Transparency.** @guenther2020german
    ...
    - **Model output:** Multi-dimensional distributional semantic transparency measures (compositional + relatedness).

### Discrimination-based models {#sec-lit-type10}
...
1. **Naive Discriminative Learning (NDL).** @baayen2011ndl
    ...
    - **Model input:** Letter n-gram cues mapped to lexical outcomes (lexomes) via Rescorla--Wagner equilibrium equations. Trained on BNC (20M words).
    - **Model output:** G2L prior (column 1-norm), activation diversity, and other discrimination-based measures.
...
2. **Discrimination in Lexical Decision.** @milin2017discrimination

    - **Dataset:** ELP (large subset); two masked priming experiments.
    - **Human DV:** Visual LD latencies.
    - **Model output:** NDL-derived measures: G2L prior, G2L activation diversity, L2L prior, L2L I-diversity.
...
3. **Discriminative Lexicon Model (DLM).** @baayen2019dlm

    - **Dataset:** ELP, BLP, DLP, MALD.
    - **Model output:** Linear Discriminative Learning (LDL)---linear mappings between orthographic/phonological form vectors and semantic vectors (word2vec). Measures: semantic density, form-to-meaning mapping strength, path certainty.
...
4. **Vector Space Morphology with LDL.** @chuang2021ldl
    ...
    - **Models tested:** LDL using fastText-style semantic vectors.
...
5. **Trial-to-Trial Learning in LD.** @heitmeier2023trial

    - **Dataset:** ELP, BLP, MALD.
    - **Human DV:** Trial-by-trial LD RT.
    - **Model output:** Widrow--Hoff incremental learning model; trial-to-trial adaptation via LDL.
...
6. **Frequency Effects in LDL.** @heitmeier2024frequency
...
## Process and decision models {#sec-lit-process-decision}

### Foundational computational models {#sec-lit-foundational}

These models provide mechanistic accounts of how perceptual evidence is processed during word recognition---through interactive activation networks, dual processing routes, or connectionist attractor dynamics.
...
2. **Triangle Model with Semantics.** @harm2004triangle
    ...
    - **Human DV:** LD RT, naming RT, accuracy.
    - **Model input:** Orthographic → phonological → semantic pathways; trained on ~6,000 words.
    - **Model output:** Semantic stress (discrepancy between computed and stored semantic representations); settlement time.
...
4. **MROM (Multiple Read-Out Model).** @grainger1996mrom
    ...
    - **Model input:** Letter strings activating orthographic word units via interactive activation.
    - **Model output:** Three read-out criteria: (1) local activity criterion—single unit exceeds threshold → "yes"; (2) temporal deadline—no unit exceeds threshold in time → "no"; (3) global activity criterion—summed activation exceeds threshold → fast "yes" based on familiarity.
...
### Evidence accumulation and decision models {#sec-lit-accumulation}

1. **The Drift-Diffusion Model.** @ratcliff1978ddm

    - **Human DV:** RT distributions + accuracy across two-choice tasks.
    - **Model output:** Parameters of a continuous random walk: drift rate $v$ (quality of evidence accumulation), boundary separation $a$ (response caution), starting point $z$ (prior bias), nondecision time $T_{er}$.
...
2. **Diffusion Model Applied to LD.** @ratcliff2004diffusion

    - **Dataset:** Six LD experiments manipulating frequency, nonword type, word proportion.
    - **Human DV:** Full RT distributions + accuracy.
    - **Model output:** Diffusion model parameters: drift rate (evidence accumulation quality), boundary separation (response caution), nondecision time.
...
6. **Linear Deterministic Accumulator Models.** @heathcote2012lba

    - **Human DV:** RT distributions, choice probabilities.
    - **Model output:** Parameters of a linear ballistic accumulator: drift rates, thresholds, start-point variability.
...
7. **The Lognormal Race.** @rouder2015lognormal

    - **Human DV:** Choice RT distributions.
    - **Model output:** Parameters of competing lognormal accumulators: means and variances of accumulation time for each response channel.
...
8. **Individual Differences.** @yap2012individual
...
## Distributional and computational models {.unnumbered}

- **LM:** Language model — a probabilistic model that assigns probabilities to sequences of words (or subword units). In this survey, the "LM" label is reserved for genuine probabilistic sequence models (e.g., GPT-family, phoneme-level surprisal models). Embedding models, discriminative learners, and corpus frequency counts use "Model" or "Corpus/source" labels instead.
